...
func (h *WebHandler) EnterprisesAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(enterprises) == 0 {
		fmt.Fprintf(w, `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No enterprises found</td></tr>`)
		return
	}
			...
			<tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
				...
				<td class="px-3 py-4 whitespace-nowrap text-right text-sm font-medium">
					<div class="flex justify-end space-x-2">
						<button hx-get="/web/enterprises/%s/edit" 
								hx-target="#modal-container" 
								class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300">Edit</button>
						<button hx-delete="/web/api/enterprises/%s" 
								hx-target="#enterprises-table" 
								hx-confirm="Are you sure you want to delete this enterprise?"
								class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300">Delete</button>
					</div>
				</td>
			</tr>`,
			enterprise.ID, enterprise.Name, forgeType, forgeIcon, enterprise.Endpoint.Name, enterprise.CredentialsName, 
			statusClass, status, enterprise.ID, enterprise.ID)
	}
}
...
func (h *WebHandler) EnterprisePoolsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(pools) == 0 {
		fmt.Fprintf(w, `<div class="px-4 py-8 text-center">
			<p class="text-sm text-gray-500 dark:text-gray-400">No pools found for this enterprise.</p>
		</div>`)
		return
	}
...
func (h *WebHandler) EnterpriseInstancesAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(instances) == 0 {
		fmt.Fprintf(w, `<div class="px-4 py-8 text-center">
			<p class="text-sm text-gray-500 dark:text-gray-400">No instances found for this enterprise.</p>
		</div>`)
		return
	}
	...
	if len(enterprise.Events) == 0 {
		w.Write([]byte(`
			<div class="px-4 py-8 text-center">
				...
				<p class="text-sm text-gray-500 dark:text-gray-400">No events found for this enterprise.</p>
