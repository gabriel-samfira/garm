...
func (h *WebHandler) EnterprisesAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, enterprise := range enterprises {
		...
		if enterprise.Endpoint.EndpointType == params.GiteaEndpointType {
			forgeType = "Gitea"
		...
		rowData := EnterpriseRowData{
			ID:              enterprise.ID,
			Name:            enterprise.Name,
			ForgeType:       forgeType,
			ForgeIcon:       forgeIcon,
			EndpointName:    enterprise.Endpoint.Name,
			CredentialsName: enterprise.CredentialsName,
			Status:          status,
			StatusClass:     statusClass,
		}

		if err := h.templates.ExecuteTemplate(w, "enterprise-table-row.html", rowData); err != nil {
			slog.Error("Failed to execute enterprise table row template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
func (h *WebHandler) EnterprisePoolsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	pools, err := h.runner.ListEnterprisePools(ctx, enterpriseID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if len(pools) == 0 {
		emptyData := EmptyContentSectionData{
			Message: "No pools found for this enterprise.",
		}
	...
	for _, pool := range pools {
		...
		if err := h.templates.ExecuteTemplate(w, "enterprise-pool-table-row.html", rowData); err != nil {
			slog.Error("Failed to execute enterprise pool table row template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
func (h *WebHandler) EnterpriseInstancesAPIHandler(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	vars := mux.Vars(r)
	enterpriseID := vars["id"]

	instances, err := h.runner.ListEnterpriseInstances(ctx, enterpriseID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if len(instances) == 0 {
		emptyData := EmptyContentSectionData{
			Message: "No instances found for this enterprise.",
		}
		if err := h.templates.ExecuteTemplate(w, "empty-content-section.html", emptyData); err != nil {
			slog.Error("Failed to execute empty content template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
	...
	for _, instance := range instances {
		...
		rowData := EnterpriseInstanceRowData{
			Name:         instance.Name,
			EntityID:     enterpriseID,
		...
		if err := h.templates.ExecuteTemplate(w, "enterprise-instance-table-row.html", rowData); err != nil {
			slog.Error("Failed to execute enterprise instance table row template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
// EnterpriseEventsAPIHandler returns events for a specific enterprise
func (h *WebHandler) EnterpriseEventsAPIHandler(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	vars := mux.Vars(r)
	enterpriseID := vars["id"]
	...
	if err != nil {
		http.Error(w, "Enterprise not found", http.StatusNotFound)
		return
	}
	...
	if len(enterprise.Events) == 0 {
		emptyData := EmptyEventsData{
			Message:    "No events found for this enterprise.",
	...
	events := make([]params.EntityEvent, len(enterprise.Events))
	copy(events, enterprise.Events)
