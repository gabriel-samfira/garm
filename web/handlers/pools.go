...
func (h *WebHandler) PoolsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, pool := range paginatedPools {
		...
		if err := h.templates.ExecuteTemplate(w, "pool-table-row.html", rowData); err != nil {
			slog.Error("Failed to execute pool table row template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
func (h *WebHandler) PoolDetailsHandler(w http.ResponseWriter, r *http.Request) {
	...
	fromEntity := r.URL.Query().Get("from")
	...
	// Create custom data structure with decoded extra specs and breadcrumb info
	data := struct {
		PageData
...
func (h *WebHandler) UpdatePoolHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.Header().Set("HX-Trigger", "poolUpdateError")
		http.Error(w, "Failed to update pool: "+err.Error(), http.StatusInternalServerError)
		return
	}

	// Close modal and trigger refresh
	w.Header().Set("HX-Trigger", "poolUpdated")
