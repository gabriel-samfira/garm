...
func (h *WebHandler) RepositoriesAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(paginatedRepos) == 0 {
		if search != "" {
			fmt.Fprintf(w, `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No repositories found matching "%s"</td></tr>`, search)
		} else {
			fmt.Fprintf(w, `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No repositories found</td></tr>`)
		}
		return
	}
	...
	for _, repo := range paginatedRepos {
		...
		fmt.Fprintf(w, `
			...
			repo.ID, repoDisplayName, forgeType, forgeIcon, repo.Endpoint.Name, repo.CredentialsName, 
