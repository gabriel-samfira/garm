...
func (h *WebHandler) OrganizationsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(paginatedOrgs) == 0 {
		if search != "" {
			fmt.Fprintf(w, `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No organizations found matching "%s"</td></tr>`, search)
		} else {
			fmt.Fprintf(w, `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No organizations found</td></tr>`)
		}
		return
	}
	...
	for _, org := range paginatedOrgs {
		...
		if org.Endpoint.EndpointType == "gitea" {
			forgeType = "Gitea"
		...
		fmt.Fprintf(w, `
			...
			org.ID, org.Name, forgeType, forgeIcon, org.Endpoint.Name, org.CredentialsName, 
			statusClass, status, org.ID, org.ID)
	}
...
func (h *WebHandler) OrganizationPoolsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(pools) == 0 {
		fmt.Fprintf(w, `<div class="px-4 py-8 text-center">
			<p class="text-sm text-gray-500 dark:text-gray-400">No pools found for this organization.</p>
		</div>`)
		return
	}
...
func (h *WebHandler) OrganizationInstancesAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(instances) == 0 {
		fmt.Fprintf(w, `<div class="px-4 py-8 text-center">
			<p class="text-sm text-gray-500 dark:text-gray-400">No instances found for this organization.</p>
		</div>`)
		return
	}
	...
	if len(org.Events) == 0 {
		w.Write([]byte(`
			<div class="px-4 py-8 text-center">
				...
				<p class="text-sm text-gray-500 dark:text-gray-400">No events found for this organization.</p>
