...
func (h *WebHandler) OrganizationsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	filteredOrgs := orgs
	if search != "" {
		filteredOrgs = []params.Organization{}
	...
	paginatedOrgs := []params.Organization{}
	if start < total {
	...
	hasMorePages := len(paginatedOrgs) == perPage && end < total
	...
	for _, org := range paginatedOrgs {
		...
		if org.Endpoint.EndpointType == params.GiteaEndpointType {
			forgeType = "Gitea"
		...
		rowData := OrganizationRowData{
			ID:              org.ID,
			Name:            org.Name,
			ForgeType:       forgeType,
			ForgeIcon:       forgeIcon,
			EndpointName:    org.Endpoint.Name,
			CredentialsName: org.CredentialsName,
			Status:          status,
			StatusClass:     statusClass,
		}
		
		if err := h.templates.ExecuteTemplate(w, "organization-table-row.html", rowData); err != nil {
			slog.Error("Failed to execute organization table row template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
func (h *WebHandler) CreateOrganizationHandler(w http.ResponseWriter, r *http.Request) {
	...
	balancerType := params.PoolBalancerType(r.FormValue("pool_balancer_type"))

	createParams := params.CreateOrgParams{
		Name:             r.FormValue("name"),
		CredentialsName:  r.FormValue("credentials_name"),
		WebhookSecret:    r.FormValue("webhook_secret"),
		PoolBalancerType: balancerType,
	}
...
func (h *WebHandler) OrganizationPoolsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	pools, err := h.runner.ListOrgPools(ctx, orgID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if len(pools) == 0 {
		emptyData := EmptyContentSectionData{
			Message: "No pools found for this organization.",
		}
	...
	for _, pool := range pools {
		...
		rowData := OrganizationPoolRowData{
			ID:           pool.ID,
			EntityID:     orgID,
			Image:        pool.Image,
		...
		if err := h.templates.ExecuteTemplate(w, "organization-pool-table-row.html", rowData); err != nil {
			slog.Error("Failed to execute organization pool table row template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
func (h *WebHandler) OrganizationInstancesAPIHandler(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	vars := mux.Vars(r)
	orgID := vars["id"]

	instances, err := h.runner.ListOrgInstances(ctx, orgID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if len(instances) == 0 {
		emptyData := EmptyContentSectionData{
			Message: "No instances found for this organization.",
		}
		if err := h.templates.ExecuteTemplate(w, "empty-content-section.html", emptyData); err != nil {
			slog.Error("Failed to execute empty content template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
	...
	for _, instance := range instances {
		...
		rowData := OrganizationInstanceRowData{
			Name:         instance.Name,
			EntityID:     orgID,
		...
		if err := h.templates.ExecuteTemplate(w, "organization-instance-table-row.html", rowData); err != nil {
			slog.Error("Failed to execute organization instance table row template", "error", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
// OrganizationEventsAPIHandler returns events for a specific organization
func (h *WebHandler) OrganizationEventsAPIHandler(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	vars := mux.Vars(r)
	orgID := vars["id"]
	...
	events := make([]params.EntityEvent, len(org.Events))
	copy(events, org.Events)
