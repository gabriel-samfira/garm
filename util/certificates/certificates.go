...
func NewPeerCertificates(name string, validFor []string, keyPath, certPath, caCert, caKey string) error {
	if _, err := os.Stat(keyPath); err == nil {
...
func NewClientCertificate(name string, keyPath, certPath, caCert, caKey string) error {
	if _, err := os.Stat(keyPath); err == nil {
		return fmt.Errorf("client key already exists")
	}

	if _, err := os.Stat(certPath); err == nil {
		return fmt.Errorf("client certificate already exists")
	}
