<script lang="ts">
	import type { Repository, Organization, Enterprise } from '$lib/api/types.js';
	...
	<div class="px-4 py-5 sm:p-6">
		...
		<dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
			...
			<div>
				...
				<dd class="mt-1">
					<span class="inline-flex px-2 py-1 text-xs font-medium rounded-full {entity.pool_manager_status?.running ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200'}">
						{entity.pool_manager_status?.running ? 'Running' : 'Stopped'}
					</span>
				</dd>
			</div>
			<div>
