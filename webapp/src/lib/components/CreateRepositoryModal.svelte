<script lang="ts">
	...
	import type { CreateRepoParams, ForgeCredentials, Endpoint, PoolBalancerType } from '$lib/api/types.js';
	...
	<div class="p-6">
		...
		{:else}
			<form on:submit|preventDefault={handleSubmit} class="space-y-4">
				...
				<!-- Repository Owner -->
				<div>
					<label for="owner" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
						Repository Owner
					</label>
					<input
						id="owner"
					...
					>
						...
						{#each filteredCredentials as credential}
							<option value={credential.name}>
								{credential.name} ({credential.endpoint?.name})
							</option>
						{/each}
					</select>
				</div>
