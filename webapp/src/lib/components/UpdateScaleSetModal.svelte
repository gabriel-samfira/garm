<script lang="ts">
	...
	let priority = scaleSet.priority;
	let runnerPrefix = scaleSet.runner_prefix || '';
	let osType = scaleSet.os_type || 'linux';
	let osArch = scaleSet.os_arch || 'amd64';
	let githubRunnerGroup = scaleSet.github_runner_group || '';
	let enabled = scaleSet.enabled;
	let tags: string[] = (scaleSet.tags || []).map(tag => typeof tag === 'string' ? tag : tag.name);
	let newTag = '';
	let extraSpecs = '{}';
	...
	onMount(() => {
		if (scaleSet.extra_specs) {
			try {
				...
				extraSpecs = scaleSet.extra_specs as string || '{}';
			}
		}
	});
	...
	async function handleSubmit() {
		try {
			...
			const params: Partial<CreateScaleSetParams> = {
				...
				priority: priority !== scaleSet.priority ? priority : undefined,
				...
				tags: JSON.stringify(tags) !== JSON.stringify((scaleSet.tags || []).map(tag => typeof tag === 'string' ? tag : tag.name)) ? tags : undefined,
			...
			<div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
					<div>
						...
						<span class="ml-2 text-gray-900 dark:text-white">{scaleSet.provider_name}</span>
					</div>
					<div>
						<span class="text-gray-500 dark:text-gray-400">Entity:</span>
						<span class="ml-2 text-gray-900 dark:text-white">
							{#if scaleSet.repo_name}{scaleSet.repo_name} (Repository)
							{:else if scaleSet.org_name}{scaleSet.org_name} (Organization)
							{:else if scaleSet.enterprise_name}{scaleSet.enterprise_name} (Enterprise)
