<script lang="ts">
	...
	import UpdateRepositoryModal from '$lib/components/UpdateRepositoryModal.svelte';
...
<div class="space-y-6">
	...
	{:else if repository}
				...
				<div class="sm:flex sm:items-center sm:justify-between">
					<div class="flex items-center space-x-3">
						<div class="flex-shrink-0">
							{@html getForgeIcon(repository.endpoint?.endpoint_type)}
						</div>
						<div>
							<h1 class="text-2xl font-bold text-gray-900 dark:text-white">{repository.name}</h1>
							<p class="text-sm text-gray-500 dark:text-gray-400">
								Owner: {repository.owner} • Endpoint: {repository.endpoint?.name}
							</p>
						</div>
					</div>
		...
		<div class="bg-white dark:bg-gray-800 shadow rounded-lg">
			<div class="px-4 py-5 sm:p-6">
				<h2 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Repository Information</h2>
				<dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">ID</dt>
						<dd class="mt-1 text-sm text-gray-900 dark:text-white font-mono">{repository.id}</dd>
					</div>
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Created At</dt>
						<dd class="mt-1 text-sm text-gray-900 dark:text-white">{formatDate(repository.created_at)}</dd>
					</div>
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Updated At</dt>
						<dd class="mt-1 text-sm text-gray-900 dark:text-white">{formatDate(repository.updated_at)}</dd>
					</div>
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Status</dt>
						<dd class="mt-1">
							<span class="inline-flex px-2 py-1 text-xs font-medium rounded-full {repository.pool_manager_status?.running ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200'}">
								{repository.pool_manager_status?.running ? 'Running' : 'Stopped'}
							</span>
						</dd>
					</div>
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Pool Balancer Type</dt>
						<dd class="mt-1 text-sm text-gray-900 dark:text-white">{repository.pool_balancer_type || 'None'}</dd>
					</div>
					<div>
						<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Repository URL</dt>
						<dd class="mt-1 text-sm">
							<a href="{repository.endpoint?.base_url}/{repository.owner}/{repository.name}" target="_blank" rel="noopener noreferrer" class="text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300">
								{repository.endpoint?.base_url}/{repository.owner}/{repository.name}
		...
		<div class="bg-white dark:bg-gray-800 shadow rounded-lg">
			<div class="px-4 py-5 sm:p-6">
				...
				{#if pools.length === 0}
					<p class="text-sm text-gray-500 dark:text-gray-400">No pools configured for this repository.</p>
				{:else}
		...
		<div class="bg-white dark:bg-gray-800 shadow rounded-lg">
			<div class="px-4 py-5 sm:p-6">
				...
				{#if instances.length === 0}
					<p class="text-sm text-gray-500 dark:text-gray-400">No instances running for this repository.</p>
				{:else}
					<div class="overflow-x-auto">
						<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
							<thead class="bg-gray-50 dark:bg-gray-700">
		...
		<!-- Events -->
		{#if repository.events && repository.events.length > 0}
			<div class="bg-white dark:bg-gray-800 shadow rounded-lg">
				<div class="px-4 py-5 sm:p-6">
					...
					<div bind:this={eventsContainer} class="space-y-3 max-h-96 overflow-y-auto scroll-smooth">
						{#each repository.events as event}
