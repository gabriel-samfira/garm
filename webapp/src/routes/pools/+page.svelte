<script lang="ts">
	...
	import type { Pool, CreatePoolParams, UpdatePoolParams, Provider } from '$lib/api/types.js';
	...
	$: filteredPools = pools.filter((pool) => {
		const entityName = getEntityName(pool);
		return entityName.toLowerCase().includes(searchTerm.toLowerCase());
	});
	$: totalPages = Math.ceil(filteredPools.length / itemsPerPage);
	...
	function getEntityName(pool: Pool): string {
		if (pool.repo_id) return pool.repo_name || 'Unknown Repository';
		if (pool.org_id) return pool.org_name || 'Unknown Organization';
		if (pool.enterprise_id) return pool.enterprise_name || 'Unknown Enterprise';
	...
	function truncateImageName(imageName: string, maxLength: number = 25): { truncated: string, isTruncated: boolean } {
	...
	<div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4">
		<div class="sm:flex sm:items-center sm:justify-between">
			<div class="flex-1 min-w-0">
				<div class="max-w-md">
					<label for="search" class="sr-only">Search pools</label>
					<div class="relative">
	...
	<div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-md">
		...
		{:else if paginatedPools.length === 0}
			<div class="p-6 text-center">
				...
				<h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No pools found</h3>
					...
					<tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
						{#each paginatedPools as pool}
							<tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
								...
								<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
									<code 
										class="bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-xs cursor-default"
										title={truncateImageName(pool.image).isTruncated ? pool.image : ''}
									>
										{truncateImageName(pool.image).truncated}
									</code>
								</td>
								<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
									{pool.flavor}
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="flex flex-col">
										<a href={getEntityUrl(pool)} class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300">
											{getEntityName(pool)}
										</a>
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="flex items-center">
										<div class="flex-shrink-0">
											{@html getForgeIcon(pool.endpoint?.endpoint_type)}
										</div>
										<div class="ml-2">
											<div class="text-sm text-gray-900 dark:text-white">{pool.endpoint?.name}</div>
										</div>
									</div>
								</td>
								<td class="px-6 py-4 whitespace-nowrap">
					...
					<div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
						<div>
							<p class="text-sm text-gray-700 dark:text-gray-300">
								...
								to <span class="font-medium">{Math.min(currentPage * itemsPerPage, filteredPools.length)}</span>
								of <span class="font-medium">{filteredPools.length}</span> results
							</p>
						</div>
						<div>
...
{#if showDeleteModal && selectedPool}
	<DeleteModal
		...
		itemName={`Pool ${selectedPool.id.slice(0, 8)}... (${getEntityName(selectedPool)})`}
