...
<svelte:head>
	<title>{pool ? `Pool ${pool.id.slice(0, 8)}... - Pool Details` : 'Pool Details'} - GARM</title>
</svelte:head>
			...
			<li>
				<div class="flex items-center">
					...
					<span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">
						{pool ? pool.id.slice(0, 8) + '...' : 'Loading...'}
					</span>
				</div>
			</li>
		</ol>
			...
			<div class="px-4 py-5 sm:p-6">
				<div class="sm:flex sm:items-center sm:justify-between">
					<div class="flex items-center space-x-3">
						...
						<div>
							<h1 class="text-2xl font-bold text-gray-900 dark:text-white font-mono">{pool.id.slice(0, 8)}...</h1>
							<p class="text-sm text-gray-500 dark:text-gray-400">
								Pool for {getEntityName(pool)} ({getEntityType(pool)})
							</p>
						</div>
					</div>
			...
			<div class="bg-white dark:bg-gray-800 shadow rounded-lg">
				<div class="px-4 py-5 sm:p-6">
					<div class="flex items-center justify-between mb-4">
						<h2 class="text-lg font-medium text-gray-900 dark:text-white">Instances ({pool.instances.length})</h2>
					...
					{#if pool.instances.length === 0}
						<p class="text-sm text-gray-500 dark:text-gray-400">No instances running for this pool.</p>
					{:else}
						<div class="overflow-x-auto">
							<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
								...
								<tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
									{#each pool.instances as instance}
										<tr>
											<td class="px-6 py-4 whitespace-nowrap">
												<a href={`${base}/instances/${encodeURIComponent(instance.name)}`} class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300">
													{instance.name}
												</a>
											</td>
											<td class="px-6 py-4 whitespace-nowrap">
...
{#if showDeleteModal && pool}
	<DeleteModal
		...
		itemName={`Pool ${pool.id.slice(0, 8)}... (${getEntityName(pool)})`}
