<script lang="ts">
	...
	$: filteredScaleSets = scaleSets.filter((scaleSet) => {
		const entityName = getEntityName(scaleSet);
		return entityName.toLowerCase().includes(searchTerm.toLowerCase());
	});
	$: totalPages = Math.ceil(filteredScaleSets.length / itemsPerPage);
	...
	async function loadScaleSets() {
		try {
			loading = true;
			error = '';
			scaleSets = await garmApi.listScaleSets();
		} catch (err) {
	...
	function handleScaleSetEvent(event: WebSocketEvent) {
		...
		if (event.operation === 'create') {
			...
			scaleSets = [...scaleSets, newScaleSet];
		} else if (event.operation === 'update') {
			const updatedScaleSet = event.payload as ScaleSet;
			scaleSets = scaleSets.map(scaleSet => 
				scaleSet.id === updatedScaleSet.id ? updatedScaleSet : scaleSet
			);
		} else if (event.operation === 'delete') {
			const scaleSetId = event.payload.id || event.payload;
			scaleSets = scaleSets.filter(scaleSet => scaleSet.id !== scaleSetId);
		}
	}
	...
	// Scale sets are GitHub only, so always show GitHub icon
	...
	onMount(() => {
		...
		// Subscribe to real-time scale set events
...
<div class="space-y-6">
	...
	<div class="sm:flex sm:items-center sm:justify-between">
		<div>
			<h1 class="text-2xl font-bold text-gray-900 dark:text-white">Scale Sets</h1>
			<p class="mt-2 text-sm text-gray-700 dark:text-gray-300">Manage GitHub runner scale sets</p>
		</div>
	...
	<div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4">
		<div class="sm:flex sm:items-center sm:justify-between">
			<div class="flex-1 min-w-0">
				<div class="max-w-md">
					<label for="search" class="sr-only">Search scale sets</label>
					<div class="relative">
	...
	<div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-md">
		{#if loading}
			<div class="p-6 text-center">
				...
				<p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Loading scale sets...</p>
			</div>
		{:else if error}
			<div class="p-6 text-center">
		...
		{:else if paginatedScaleSets.length === 0}
			<div class="p-6 text-center">
				<svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
				</svg>
				<h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No scale sets found</h3>
				<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
					{#if searchTerm}
						No scale sets match your search criteria.
					{:else}
						Get started by creating your first scale set.
					{/if}
				</p>
			</div>
		{:else}
			<div class="overflow-x-auto">
				<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
					...
					<tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
						{#each paginatedScaleSets as scaleSet}
							<tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
								<td class="px-6 py-4 whitespace-nowrap">
									<a href={`${base}/scalesets/${scaleSet.id}`} class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300">
										{scaleSet.name}
									</a>
								</td>
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="flex items-center">
										<div class="flex-shrink-0">
											{@html getForgeIcon(scaleSet.endpoint?.endpoint_type)}
										</div>
										<div class="ml-2">
											<div class="text-sm text-gray-900 dark:text-white">{scaleSet.endpoint?.name || 'Unknown'}</div>
										</div>
									</div>
								</td>
								<td class="px-6 py-4 whitespace-nowrap">
								...
								<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
									<div class="flex justify-end space-x-2">
										<button
											on:click={() => openUpdateModal(scaleSet)}
											class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300"
											title="Edit scale set"
										>
										...
										<button
											on:click={() => openDeleteModal(scaleSet)}
					...
					<div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
						<div>
							<p class="text-sm text-gray-700 dark:text-gray-300">
								...
								to <span class="font-medium">{Math.min(currentPage * itemsPerPage, filteredScaleSets.length)}</span>
								of <span class="font-medium">{filteredScaleSets.length}</span> results
							</p>
						</div>
						<div>
