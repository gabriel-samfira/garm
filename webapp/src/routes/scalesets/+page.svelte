<script lang="ts">
	...
	// Subscribe to eager cache for scale sets
	...
	onMount(async () => {
		...
		try {
			await eagerCacheManager.getScaleSets();
		} catch (err) {
				...
				<div class="rounded-md bg-red-50 dark:bg-red-900 p-4">
					<div class="flex">
						...
						<div class="ml-3 flex-1">
							<h3 class="text-sm font-medium text-red-800 dark:text-red-200">Error loading scale sets</h3>
							<p class="mt-2 text-sm text-red-700 dark:text-red-300">{cacheError || error}</p>
							{#if cacheError}
								<div class="mt-3">
									<button
										on:click={retryLoadScaleSets}
