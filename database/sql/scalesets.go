...
func (s *sqlDatabase) AddScaleSetEvent(ctx context.Context, scaleSet uint, event params.EventType, eventLevel params.EventLevel, statusMessage string) error {
	err := s.conn.Transaction(func(tx *gorm.DB) error {
		scaleSet, err := s.getScaleSetByID(tx, scaleSet)
		if err != nil {
		...
		if err := tx.Model(&scaleSet).Association("StatusMessages").Append(&msg); err != nil {
