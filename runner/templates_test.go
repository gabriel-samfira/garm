...
func (s *TemplateTestSuite) TestRestoreTemplateSpecific() {
	...
	s.Require().Equal("system", restoredTemplate.Owner)
...
func (s *TemplateTestSuite) TestRestoreTemplateAll() {
	...
	for templateID := range modifiedTemplateIDs {
		...
		s.Require().Equal("system", template.Owner)
...
func (s *TemplateTestSuite) TestRestoreTemplatePreservesUserTemplates() {
	...
	s.Require().Nil(err)
	s.Require().NotEqual("system", userTemplate.Owner, "User template should not be system owned")
	...
	s.Require().NotEqual("system", userTemplateAfter.Owner)
}
