...
func (r *Runner) ListGARMTools(ctx context.Context) ([]params.GARMAgentTool, error) {
	...
	windowsAMD64Tools, err := r.store.SearchFileObjectByTags(r.ctx, windowsAMD64ToolsTags, 1, 10)
	if err != nil {
		return nil, fmt.Errorf("failed to list files: %w", err)
	}
	windowsARM64Tools, err := r.store.SearchFileObjectByTags(r.ctx, windowsARM64ToolsTags, 1, 10)
	if err != nil {
		return nil, fmt.Errorf("failed to list files: %w", err)
	}
	linuxAMD64Tools, err := r.store.SearchFileObjectByTags(r.ctx, linuxAMD64ToolsTags, 1, 10)
	if err != nil {
		return nil, fmt.Errorf("failed to list files: %w", err)
	}
	linuxARM64Tools, err := r.store.SearchFileObjectByTags(r.ctx, linuxARM64ToolsTags, 1, 10)
	if err != nil {
		return nil, fmt.Errorf("failed to list files: %w", err)
	}

	ret := []params.GARMAgentTool{}
	if windowsAMD64Tools.TotalCount > 0 && len(windowsAMD64Tools.Results) > 0 {
		tools, err := fileObjectToGARMTool(windowsAMD64Tools.Results[0], "")
		if err != nil {
			return nil, fmt.Errorf("failed to parse object with ID %d", windowsAMD64Tools.Results[0].ID)
		}
		ret = append(ret, tools)
	}

	if windowsARM64Tools.TotalCount > 0 && len(windowsARM64Tools.Results) > 0 {
		tools, err := fileObjectToGARMTool(windowsARM64Tools.Results[0], "")
		if err != nil {
			return nil, fmt.Errorf("failed to parse object with ID %d", windowsARM64Tools.Results[0].ID)
		}
		ret = append(ret, tools)
	}

	if linuxAMD64Tools.TotalCount > 0 && len(linuxAMD64Tools.Results) > 0 {
		tools, err := fileObjectToGARMTool(linuxAMD64Tools.Results[0], "")
		if err != nil {
			return nil, fmt.Errorf("failed to parse object with ID %d", linuxAMD64Tools.Results[0].ID)
		}
		ret = append(ret, tools)
	}

	if linuxARM64Tools.TotalCount > 0 && len(linuxARM64Tools.Results) > 0 {
		tools, err := fileObjectToGARMTool(linuxARM64Tools.Results[0], "")
		if err != nil {
			return nil, fmt.Errorf("failed to parse object with ID %d", linuxARM64Tools.Results[0].ID)
		}
		ret = append(ret, tools)
	}
	return ret, nil
